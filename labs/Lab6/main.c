...
static ROWCOL CreatePath(ROWCOL rowcol)
{
  ...
  while (((nibble = GetNibble(flags, which)) & DIR_ALL4) != 0)
  {
    ...
    case DIR_RIGHT:
      if (COL(rowcol) + CELL_INCR < CELL_COLS)
        next_col = COL(rowcol) + CELL_INCR;
      else
        continue;
      break;
    ...
    case DIR_DOWN:
      if (ROW(rowcol) + CELL_INCR < CELL_ROWS)
        next_row = ROW(rowcol) + CELL_INCR;
      else
        continue;
      break;
...
#define NIBBLES(a) (4 * WORDS(a))
  ...
  for (int i = 0; i < WORDS(storage); i++)
    storage[i] = 0;

  bugs = 0;

  do
    index = GetRandomNumber() % NIBBLES(storage);
  while (index < 8);
  PutNibble(storage, index, 0xF);
  word = index / 8;
  left = index % 8;
  if (storage[word] != (0xF << 4 * left))
    bugs |= 0x1;
  storage[word] = 0;
  ...
  if (GetNibble(storage, index) != 0xF)
    bugs |= 0x2;
  storage[word] = 0;
  ...
  if (bugs & 0x1)
    DisplayStringAt(5, 50, (uint8_t *)" Bad Function PutNibble\n");
  if (bugs & 0x2)
    DisplayStringAt(5, 70, (uint8_t *)" Bad Function GetNibble\n");
  return 0;
}
...
static void GetVelocity(float degrees_per_sec[])
{
  ...
  for (channel = 0; channel < 3; channel++)
  {
    ...
    if (count < ZERO_RATE_SAMPLES)
    {
      ...
      if (sample > max[channel])
        max[channel] = sample + 1;
      if (sample < min[channel])
        min[channel] = sample - 1;
      count++;
    }

    // subtract zero-rate-level and compute angular velocity
    if (min[channel] <= sample && sample <= max[channel])
      value = 0;
    else
      value = sample - total[channel] / count;
