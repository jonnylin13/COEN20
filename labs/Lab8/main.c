...
static float HWFlPtSine(float radians, unsigned ovhd)
{
  ...
  {
    if (!error)
      PutErrMsgAt(HWFP_TXTROW, MSG_TXTCOL, "H/W FlPt: sin(%4.2f rad) < 0", radians);
    error = 1;
    sine = 0.0;
  }
  else if (sine > 1.0)
  {
    if (!error)
      PutErrMsgAt(HWFP_TXTROW, MSG_TXTCOL, "H/W FlPt: sin(%4.2f rad) > 1", radians);
    error = 1;
    sine = 1.0;
  }
...
static float SWFlPtSine(float radians, unsigned ovhd)
{
  ...
  {
    if (!error)
      PutErrMsgAt(SWFP_TXTROW, MSG_TXTCOL, "S/W FlPt: sin(%4.2f rad) < 0", radians);
    error = 1;
    sine = 0.0;
  }
  else if (sine > 1.0)
  {
    if (!error)
      PutErrMsgAt(SWFP_TXTROW, MSG_TXTCOL, "S/W FlPt: sin(%4.2f rad) > 1", radians);
    error = 1;
    sine = 1.0;
  }
  else if (!error && radians == 0.0)
...
static float Q16FxdSine(float radians, unsigned ovhd)
{
  ...
  {
    if (!error)
      PutErrMsgAt(FXPT_TXTROW, MSG_TXTCOL, "Q16 FxPt: sin(%4.2f rad) < 0", radians);
    error = 1;
    sine = 0;
  }
  else if (sine > one)
  {
    if (!error)
      PutErrMsgAt(FXPT_TXTROW, MSG_TXTCOL, "Q16 FxPt: sin(%4.2f rad) > 1", radians);
    error = 1;
    sine = one;
  }
